        ...
        <div className="text-left text-sm">
            <ul>
                {groups.map((group)=> {
                  return (
                <Link key={group._id}>
                <li key={group._id} className={`hover:bg-gray-300 pl-2 py-1 border-l-4 ${location.pathname==='/abc' ? "border-blue-300 text-blue-300" : "border-transparent"}`}><i className="fa-solid fa-user-group mr-2 text-xs"></i>{group.title}</li>
                </Link>
                  )
                })}
